...
const SectionForm: React.FC<{ sectionKey: string; title: string; description?: string; fields: any[]; initial: any }> = ({ sectionKey, title, description, fields, initial }) => {
    ...
    <Card variant="outlined"><CardContent>
      ...
      <Grid container spacing={2}>
        {fields.map((f: any) => (<Grid size={{ xs: 12, md: f.full ? 12 : 6 }} key={f.name}>
          {f.type === 'switch' ? (<FormControlLabel control={<Switch checked={!!form[f.name]} onChange={e => setForm({ ...form, [f.name]: e.target.checked })} />} label={f.label} />)
            : f.type === 'select' ? (<TextField select fullWidth label={f.label} value={form[f.name] ?? ''} onChange={e => setForm({ ...form, [f.name]: e.target.value })}>{(f.options || []).map((o: any) => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}</TextField>)
...
export const ChurchSettingsPage: React.FC = () => {
  ...
  const tabs = ['Church Profile & Logo', 'Branches', 'Service Schedules', 'Financial Year', 'SMS', 'Email', 'Payment Gateway', 'Integration', 'Backup & Restore'];
  const g = s.general || {};
  return (
    <Box>
