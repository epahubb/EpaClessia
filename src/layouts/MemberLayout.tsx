...
const MemberLayout: React.FC = () => {
  ...
  const menuItems = [
    { text: 'Dashboard', icon: <LayoutDashboard size={20} />, path: '/member/dashboard' },
    ...
    { text: 'Directory', icon: <BookOpen size={20} />, path: '/member/directory' },
  ];
        ...
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box sx={{ bgcolor: 'info.main', p: 0.5, borderRadius: 1, display: 'flex' }}><ShieldCheck size={20} color="white" /></Box>
              <Typography variant="h6" noWrap fontWeight={700} sx={{ letterSpacing: '-0.5px' }}>Ecclesia</Typography>
              <Chip label="MEMBER" size="small" sx={{ ml: 1, height: 18, fontSize: '0.65rem', fontWeight: 700, bgcolor: 'info.main', color: 'white' }} />
            </Box>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Avatar sx={{ width: 32, height: 32, bgcolor: 'info.main', fontSize: '0.875rem' }}>{user?.name?.[0] || 'U'}</Avatar>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <ListItem key={item.text} disablePadding sx={{ display: 'block', mb: 0.5 }}>
                  <ListItemButton onClick={() => { navigate(item.path); if (isMobile) setOpen(false); }} sx={{ minHeight: 48, justifyContent: open ? 'initial' : 'center', px: 2.5, borderRadius: 2, bgcolor: isActive ? 'rgba(2, 132, 199, 0.08)' : 'transparent', color: isActive ? 'info.main' : 'text.secondary' }}>
                    <ListItemIcon sx={{ minWidth: 0, mr: open ? 2 : 'auto', justifyContent: 'center', color: isActive ? 'info.main' : 'inherit' }}>{item.icon}</ListItemIcon>
