...
import { 
  ...
  BarChart3, LogOut, Bell, ShieldCheck
} from 'lucide-react';
...
const MinistryLeaderLayout: React.FC = () => {
  ...
  const menuItems = [
    { text: 'Dashboard', icon: <LayoutDashboard size={20} />, path: '/ministry/dashboard' },
    { text: 'Members', icon: <Users size={20} />, path: '/ministry/members' },
    { text: 'Tasks', icon: <CheckSquare size={20} />, path: '/ministry/tasks' },
    { text: 'Events', icon: <Calendar size={20} />, path: '/ministry/events' },
    { text: 'Communication', icon: <MessageSquare size={20} />, path: '/ministry/communication' },
    { text: 'Reports', icon: <BarChart3 size={20} />, path: '/ministry/reports' },
  ];
        ...
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box sx={{ bgcolor: 'success.main', p: 0.5, borderRadius: 1, display: 'flex' }}><ShieldCheck size={20} color="white" /></Box>
              <Typography variant="h6" noWrap fontWeight={700} sx={{ letterSpacing: '-0.5px' }}>Ecclesia</Typography>
              <Chip label="MINISTRY LEADER" size="small" sx={{ ml: 1, height: 18, fontSize: '0.65rem', fontWeight: 700, bgcolor: 'success.main', color: 'white' }} />
            </Box>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Avatar sx={{ width: 32, height: 32, bgcolor: 'success.main', fontSize: '0.875rem' }}>{user?.name?.[0] || 'M'}</Avatar>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <ListItem key={item.text} disablePadding sx={{ display: 'block', mb: 0.5 }}>
                  <ListItemButton onClick={() => { navigate(item.path); if (isMobile) setOpen(false); }} sx={{ minHeight: 48, justifyContent: open ? 'initial' : 'center', px: 2.5, borderRadius: 2, bgcolor: isActive ? 'rgba(16, 185, 129, 0.08)' : 'transparent', color: isActive ? 'success.main' : 'text.secondary' }}>
                    <ListItemIcon sx={{ minWidth: 0, mr: open ? 2 : 'auto', justifyContent: 'center', color: isActive ? 'success.main' : 'inherit' }}>{item.icon}</ListItemIcon>
...
export default MinistryLeaderLayout;
